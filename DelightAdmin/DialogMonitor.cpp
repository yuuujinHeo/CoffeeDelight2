...
DialogMonitor::DialogMonitor(QWidget *parent, DialogStock *_stock, DisplayDialog *_display) :
    QDialog(parent), stock(_stock), display(_display),
...
//    }
    ...
    if(ui->CB_FORCE_MONITOR_STATUS->isChecked()){
        display->notice = ui->LE_FORCE_MSG->text();
        display->notice_count = 3;
...
//    }
    ...
    for(int i=0; i<pschedule->list_inprogress.size(); i++){
        QString pin = pschedule->list_inprogress[i].pin;
        QString marking = "■";
        QString space = "  ";
        QString menu_name = pschedule->list_inprogress[i].menu_name;
    ...
    for(int i=0; i<pschedule->list_waiting.size(); i++){
        QString pin = pschedule->list_waiting[i].pin;
        QString marking = "□";
        QString space = "  ";
        QString menu_name = pschedule->list_waiting[i].menu_name;
        QString statement = marking + space + "[" + pin + "]" + space + menu_name;
        ui->LW_WAITING->addItem(statement);
    }
    ...
    if(pschedule->list_dispensing[0].pin != ""){
        QString pin = pschedule->list_dispensing[0].pin;
        QString marking = "■";
        QString space = "  ";
        QString menu_name = pschedule->list_dispensing[0].menu_name;
//        QString id_module = QString::number(pschedule->list_dispensing[0].id_module);
        QString id_cell = QString::number(pschedule->list_dispensing[0].id_cell);
//        QString statement = marking + space + "[" + pin + "]" + space + "[" + id_module + " | " + id_cell + "]" + space + menu_name;
        QString statement = marking + space + "[" + pin + "]" + space + "[" + id_cell + "]" + space + menu_name;
        ui->LW_COMPLETED->addItem(statement);
    }
    if(pschedule->list_dispensing[1].pin != ""){
        QString pin = pschedule->list_dispensing[1].pin;
        QString marking = "■";
        QString space = "  ";
        QString menu_name = pschedule->list_dispensing[1].menu_name;
//        QString id_module = QString::number(pschedule->list_dispensing[1].id_module);
        QString id_cell = QString::number(pschedule->list_dispensing[1].id_cell);
//        QString statement = marking + space + "[" + pin + "]" + space + "[" + id_module + " | " + id_cell + "]" + space + menu_name;
        QString statement = marking + space + "[" + pin + "]" + space + "[" + id_cell + "]" + space + menu_name;
        ui->LW_COMPLETED->addItem(statement);
    }
    for(int i=0; i<pschedule->list_completed.size(); i++){
        QString pin = pschedule->list_completed[i].pin;
        QString marking = "□";
        QString space = "  ";
        QString menu_name = pschedule->list_completed[i].menu_name;
//        QString id_module = QString::number(pschedule->list_completed[i].id_module);
        QString id_cell = QString::number(pschedule->list_completed[i].id_cell);
//        QString statement = marking + space + "[" + pin + "]" + space + "[" + id_module + " | " + id_cell + "]" + space + menu_name;
        QString statement = marking + space + "[" + pin + "]" + space + "[" + id_cell + "]" + space + menu_name;
        ui->LW_COMPLETED->addItem(statement);
    }
...
void DialogMonitor::ProvideContextList(const QPoint &pos){
    ...
    if(ui->LW_WAITING->indexAt(pos).row() < pschedule->list_inprogress.size()+pschedule->list_waiting.size() && ui->LW_WAITING->indexAt(pos).row() > -1){
        QMenu submenu1;
        submenu1.addAction("Delete");
        submenu1.addAction("Delete All");
        QAction* rightClickItem = submenu1.exec(item);
        int Inprogress_size = pschedule->list_inprogress.size();
...
void DialogMonitor::on_BTN_FORCE_OPERATION_clicked(){
    ...
    if(ui->CB_FORCE_MONITOR_STATUS->isChecked()){
        display->Set_ForcedMode(DISPLAY_MODE_IN_OPERATION);
...
void DialogMonitor::on_BTN_FORCE_PREPARATION_clicked(){
    force_monitor_status = "IN_READY";
    if(ui->CB_FORCE_MONITOR_STATUS->isChecked()){
        display->Set_ForcedMode(DISPLAY_MODE_IN_READY);
...
void DialogMonitor::on_CB_FORCE_MONITOR_STATUS_clicked(bool checked){
    if(checked == true){
        ...
        display->Set_ForcedMode(DISPLAY_MODE_IDLE);
