...
void Scheduler::onTimer(){
    ...
    static int not_cleared_count = 0;
...
//    monitor->UpdateList_DispensingDrinkgs(list_dispensing);
    ...
    case ACTION_STATE_GET_ORDER:{
        ...
        if(coffee->IsRefillMilkSoon()){
            ...
            display->Set_Notice("위생적인 음료 제조를 위해 우유관을 청소하고 있습니다. 커피머신을 이용하지 않는 음료를 먼저 제조합니다.(5분 소요)");
...
//                }
    ...
    case ACTION_STATE_CUP_ERROR:
        ...
        display->Set_Notice("컵의 재고가 없습니다. 관리자를 호출해주세요^^");
        break;
...
//        }
    ...
    case ACTION_STATE_HOT_READY:
        ...
        if(hot_toggle%2 == 0){
            if(hot->IsHotDev_1_Available() == true){
                hot_dev_selection = 1;
            }else{
                if(hot->IsHotDev_2_Available() == true){
                    hot_dev_selection = 2;
                }else{
                    // no hot device available
                    // fatal error
                    qDebug() << "no hot device available";
                    break;
                }
            }
        }else{
    ...
    case ACTION_STATE_HOT_DISPENSE:
    {
        ...
        timeout = (time_ms+1000)/SCHEDULER_TIME_INTERVAL;
    ...
    case ACTION_STATE_HOT_WAIT_DISPENSE_DONE:
        ...
        if(hot_dev_selection == 1){
            ...
            if(HOT_DATA[1].prev_out_count != HOT_DATA[1].out_count){
                action_state = ACTION_STATE_HOT_GET;
            }
        }
        break;
    ...
    case ACTION_STATE_COFFEE_RINSE:
    {
        ...
        if(current_recipe_step.amount == "ESP" && coffee->isRinseafterMilk){
            if(coffee->last_made_beverage != stock->coffee_key_btn["ESP"].toInt()){
                ...
                display->Set_Notice("커피머신 청소중입니다. 잠시만 기다려주세요.");
...
//        qDebug() << "ACTION_STATE_COFFEE_WAIT_DISPENSE_DONE";
        ...
        if(coffee_command_check_count > 10000/SCHEDULER_TIME_INTERVAL){
            if(coffee->coffee_command_flag == true){
                if(not_cleared_count > 2){
                    plog->write("[SCHEDULE COFFEE] COFFEE COMMAND FLAG NOT CLEARED!!!! -> PASS");
                    not_cleared_count = 0;
...
//        break;
    ...
    case ACTION_STATE_OUTLET_READY:
        ...
        if(outlet_cell_info == -1){
            qDebug()<<"ALL MODULES ARE RUNNiNG OR CELLS ARE OCCUPIED";
            display->Set_Notice("모든 배출구에 음료가 찼습니다. 관리자를 호출해주세요^^");
            break;
        }

//        if(outlet->IsCellWorking(outlet_cell_info) == true){
//            qDebug() << "Outlet is working.. Wait.." << outlet_cell_info;
//            break;
//        }
        display->Set_Notice("");

        outlet->SetCellWorking(outlet_cell_info, true);
        outlet->SetCellOccupancy(outlet_cell_info, CELL_OCC, list_inprogress[0].pin);
    ...
    case ACTION_STATE_OUTLET_CUP_PLACE:
        ...
        if(outlet_cell_info == 0) keymotion = "OUT_OUTLET_1";
        else if(outlet_cell_info == 1) keymotion = "OUT_OUTLET_2";
        else if(outlet_cell_info == 2) keymotion = "OUT_OUTLET_3";
        else if(outlet_cell_info == 3) keymotion = "OUT_OUTLET_4";
        else if(outlet_cell_info == 4) keymotion = "OUT_OUTLET_5";
    ...
    case ACTION_STATE_OUTLET_GET:
    {
        ...
        outlet->SetCellWorking(outlet_cell_info, false);
        ...
        temp_info.id_cell = outlet_cell_info;
        ...
        plog->write("[SCHEDULE MOTION] COMPLETE MAKING: PIN[" + temp_info.pin + "] / NAME[" + temp_info.menu_name +
                    "] --> OUTLET[" + QString::number(temp_info.id_cell+1) + "]");
    ...
    if(barcode->NewInputNotification == true){
        ...
        for(int i=0; i<list_completed.size();i++){
            if(list_completed[i].pin == barcode_pin){
                ...
                int id_cell = list_completed[i].id_cell;
                if(outlet->IsCellWorking(id_cell) == false){

                    list_dispensing[id_cell].pin = list_completed[i].pin;
                    list_dispensing[id_cell].menu_id = list_completed[i].menu_id;
                    list_dispensing[id_cell].menu_name = list_completed[i].menu_name;

                    list_dispensing[id_cell].id_cell = list_completed[i].id_cell;

                    plog->write("[SCHEDULE OUTLET] DISPENSE REQUEST: PIN[" + list_completed[i].pin + "] / NAME[" + list_completed[i].menu_name +
                                "] --> OUTLET[" + QString::number(list_completed[i].id_cell+1) + "]");

                    outlet_state[id_cell] = OUTLET_MODULE_READY;
...
//            if(outlet->IsDoorClosed(i) == true){  // new added by lim -- motion miss prevention
//                outlet_state[i] = OUTLET_MODULE_DOOR_OPEN;
//            }

//            outlet_state[i] = OUTLET_MODULE_WAIT_DRINK_REMOVED;
//            break;

//        case OUTLET_MODULE_WAIT_DRINK_REMOVED:

//            if((outlet->IsDrinkExist(i) == true) || (outlet->IsDoorClosed(i) == false)){
//                break;
//            }

//            outlet_state[i] = OUTLET_MODULE_TRY_DOOR_CLOSE;
//            break;

//        case OUTLET_MODULE_TRY_DOOR_CLOSE:
//            if((outlet->IsDrinkExist(i) == false) && (outlet->IsDoorClosed(i) == true)){
//                outlet->LockDoor(i);
//                outlet_state[i] = OUTLET_MODULE_CHECK_DOOR_CLOSED;
//            }else{
//                outlet_state[i] = OUTLET_MODULE_WAIT_DRINK_REMOVED;
...
//            break;
        ...
        case OUTLET_MODULE_CHECK_DOOR_CLOSED:
            if(--timeout_door[i] > 0)
                break;

            if(outlet->IsDoorLocked(i) == true){
                if((outlet->IsDrinkExist(i) == false) && (outlet->IsDoorClosed(i) == true)){
...
//                    outlet_state[i] = OUTLET_MODULE_DOOR_OPEN;
        ...
        case OUTLET_MODULE_CHECK_CELL_OCC:
            ...
            outlet->SetCellOccupancy(i, CELL_EMP, "");
            outlet->SetCellWorking(i, false);
        ...
        case OUTLET_MODULE_COMPLETED:
            plog->write("[SCHEDULE OUTLET] DISPENSE COMPLETE: PIN[" + list_dispensing[i].pin + "] / NAME[" + list_dispensing[i].menu_name +
                        "] --> OUTLET[" + QString::number(list_dispensing[i].id_cell+1) + "]");
...
void Scheduler::SetForcedDispensing(int id_cell){
    if(outlet->IsCellWorking(id_cell) == false){
        list_dispensing[id_cell].pin = OUTLET_DATA.cell_occupy_pin[id_cell];
        outlet_state[id_cell] = OUTLET_MODULE_READY;
        ...
        for(int i=0; i<list_completed.size(); i++){
            if(list_completed[i].id_cell == id_cell){
                list_completed.removeAt(i);
            }
        }
    }else{
