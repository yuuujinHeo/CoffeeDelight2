...
HOT_DISPENSER_INFO      HOT_DATA[2];
BARCODE_INFO            BARCODE_DATA[1];
OUTLET_INFO             OUTLET_DATA;
    ...
    ui(new Ui::MainWindow)
{
    ...
    monitor = new DialogMonitor(ui->FRAME_DEVICE, stock, display);
    ...
    hot = new DialogHotDispenser(ui->FRAME_DEVICE, stock);
    ...
    setting = new DialogSetting(ui->FRAME_DEVICE, stock, robot, cup, hot, coffee, ice, outlet, barcode);
    setting->setWindowFlags(Qt::Widget);
    setting->move(0, 0);


    schedule = new Scheduler(stock, robot, cup, hot, coffee, ice, outlet, barcode, display);
    pschedule = schedule;
    ...
    case PLATFORM_INIT_OUTLET:
        ...
        for(int i=0; i<NUMBER_OF_OUTLET; i++){
            if(outlet->OUTLET_DISABLE[i] == 0){
                outlet->init_state[i] = OUTLET_INIT_START;
            }
        }
        timeout = 1000/MAINWINDOW_LOOP_TIME;
...
void MainWindow::CoreLogic_Operating(){
    ...
    case PLATFORM_OPERATION_CHECK_OUTLET_OCCUPY:
    {
        ...
        int occupy_num = 0;
        for(int i=0; i<NUMBER_OF_OUTLET; i++){
            if(outlet->OUTLET_DISABLE[i] == 0 && OUTLET_DATA.cell_state[i] == CELL_OCC){
                occupy_num++;
            }
        }
        if(occupy_num > 0){
...
//    return;
    ...
    if(OP_STATUS_OPERATING == PLATFORM_OPERATION_ON_OPERATING_STATE){
        ...
        display->Set_Mode(DISPLAY_MODE_IN_OPERATION);
        display->notice = "에러가 발생하였습니다. 관리자를 호출해주세요.";
    }else{
        display->notice = "";
...
void MainWindow::Update_Dev_HotInfo(){
    if(hot->isOpenPort1() == false){
        ...
        if(HOT_DATA[0].connection_status == 1 && HOT_DATA[0].data_connection == 1){
            ...
            ui->LB_HOT_LAST_ERROR->setText(hot->hot_1_error);

            if(hot->state_1 == HOT_STATE_IDLE){
    ...
    if(hot->isOpenPort2() == false){
        SetBTNColor(ui->BTN_DEV_INFO_HOT_2, COLOR_UNDETERMINED);
        ui->LB_HOT_LAST_ERROR_2->setText("-");
        ui->LB_HOT_OP_STATE_2->setText("-");
    }else{
        if(HOT_DATA[1].connection_status == 1 && HOT_DATA[1].data_connection == 1){
            SetBTNColor(ui->BTN_DEV_INFO_HOT_2, COLOR_GOOD);

            ui->LB_HOT_LAST_ERROR_2->setText(hot->hot_2_error);

            if(hot->state_2 == HOT_STATE_IDLE){
                ui->LB_HOT_OP_STATE_2->setText("준비");
            }else{
                ui->LB_HOT_OP_STATE_2->setText("동작중");
            }
        }else{
            SetBTNColor(ui->BTN_DEV_INFO_HOT_2, COLOR_WEAKBAD);
            ui->LB_HOT_LAST_ERROR_2->setText("-");
            ui->LB_HOT_OP_STATE_2->setText("-");
        }
    }
...
void MainWindow::on_BTN_DEV_INFO_HOT_2_clicked(){
    ChangeDeviceDialog(DEV_DIALOG_ID_HOT);
}
