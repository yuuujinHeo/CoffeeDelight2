...
extern HOT_DISPENSER_INFO  HOT_DATA[2];
...
typedef enum{
    ...
    ODS_OPEN_MOTOR_OPEN,
    ODS_OPEN_WAIT_OPEN,
    ODS_OPEN_WAIT_CUP_REMOVE,
    ODS_CLOSE_START,
    ODS_CLOSE_MOTOR_CLOSE,
    ODS_CLOSE_WAIT_CLOSE,
    ODS_CLOSE_WAIT_SENSOR_CHECK,
    ODS_CLOSE_EM_LOCK
}OUTLET_DEV_STATE;
...
typedef struct{
    ...
    unsigned char       state[NUMBER_OF_OUTLET];

    unsigned char       is_drink_sensor_on[NUMBER_OF_OUTLET];   // is sensor detected?
    unsigned char       is_locked[NUMBER_OF_OUTLET];        // door locked? em-lock signal

//    unsigned char       is_closed[NUMBER_OF_OUTLET];        // door sensor
    unsigned char       cell_state[NUMBER_OF_OUTLET];       // occupy / empty
    QString             cell_occupy_pin[NUMBER_OF_OUTLET];
    unsigned char       is_working[NUMBER_OF_OUTLET];       // software process occupied
}OUTLET_INFO;
extern OUTLET_INFO  OUTLET_DATA;
